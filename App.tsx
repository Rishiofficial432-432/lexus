...
const App: React.FC = () => {
  // --- STATE MANAGEMENT ---
  const [pages, setPages] = usePersistentState<Page[]>('lexus-pages', []);
  const [journalEntries, setJournalEntries] = usePersistentState<JournalEntry[]>('lexus-journal', []);
  const [activePageId, setActivePageId] = useState<string | null>(localStorage.getItem('lexus-last-page-id'));
  const [view, setView] = usePersistentState<View>('lexus-view', 'notes');
  const [activeDashboardTab, setActiveDashboardTab] = usePersistentState<string>('lexus-dashboard-tab', 'dashboard');
  
  // Dashboard states
  const [tasks, setTasks] = usePersistentState<Task[]>('lexus-tasks', []);
  const [kanbanColumns, setKanbanColumns] = usePersistentState<KanbanState>('lexus-kanban', {
    todo: { name: 'To Do', items: [] },
    progress: { name: 'In Progress', items: [] },
    done: { name: 'Done', items: [] },
  });
  const [quickNotes, setQuickNotes] = usePersistentState<QuickNote[]>('lexus-quicknotes', []);
  const [events, setEvents] = usePersistentState<CalendarEvent[]>('lexus-events', []);
  const [habits, setHabits] = usePersistentState<Habit[]>('lexus-habits', []);
  const [personalQuotes, setPersonalQuotes] = usePersistentState<Quote[]>('lexus-quotes', []);
  const [moodEntries, setMoodEntries] = usePersistentState<MoodEntry[]>('lexus-moods', []);
  const [expenses, setExpenses] = usePersistentState<Expense[]>('lexus-expenses', []);
  const [goals, setGoals] = usePersistentState<Goal[]>('lexus-goals', []);
  
  // Pomodoro Timer
  const [pomodoroTime, setPomodoroTime] = useState(25 * 60);
  const [pomodoroActive, setPomodoroActive] = useState(false);
  const [pomodoroSessions, setPomodoroSessions] = usePersistentState<number>('lexus-pomodoro-sessions', 0);
  const timerRef = useRef<number | null>(null);
  
  // Decision Maker
  const [decisionOptions, setDecisionOptions] = usePersistentState<string[]>('lexus-decision-options', []);
  ...
  // Theme
  const [theme, setTheme] = usePersistentState<string>('lexus-theme', 'theme-dark');

  // Sidebar & Chatbot Collapse State
  const [isSidebarCollapsed, setIsSidebarCollapsed] = usePersistentState<boolean>('lexus-sidebar-collapsed', false);
  const [isChatbotCollapsed, setIsChatbotCollapsed] = usePersistentState<boolean>('lexus-chatbot-collapsed', false);
  
  // Google Workspace State
  const [googleAuthToken, setGoogleAuthToken] = usePersistentState<any | null>('lexus-google-token', null);
  const [workspaceHistory, setWorkspaceHistory] = usePersistentState<WorkspaceHistoryEntry[]>('lexus-workspace-history', []);
  
  // Search Palette State
  const [isSearchOpen, setIsSearchOpen] = useState(false);
  
  // App Load State
  const [hasEntered, setHasEntered] = useState(sessionStorage.getItem('lexus-has-entered') === 'true');
  ...
  const [inspirationImageId, setInspirationImageId] = usePersistentState<string | null>('lexus-inspiration-image-id', null);
  ...
  const handleEnterApp = () => {
    setHasEntered(true);
    sessionStorage.setItem('lexus-has-entered', 'true');
  };

  const handleGoToLandingPage = () => {
    setHasEntered(false);
    sessionStorage.removeItem('lexus-has-entered');
  };
  ...
  const onSelectPage = (id: string) => {
    setActivePageId(id);
    localStorage.setItem('lexus-last-page-id', id);
    setView('notes');
  };
...
[Main Content Area]
  ...
  const handleExportData = () => {
    ...
    link.download = `lexus_backup_${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  const handleWipeData = () => {
    if (dataWipeConfirmation.toLowerCase() === 'delete my data') {
        localStorage.clear();
        indexedDB.deleteDatabase('LexusDB');
        // Also clear portal DB
        indexedDB.deleteDatabase('LexusPortalDB');
        ...
        settings: (
          <div className="max-w-4xl mx-auto space-y-8">
            <Section title="API Configuration">
              <p className="text-card-foreground/80 -mt-4 mb-6">Lexus uses Google AI for its intelligent features. Your API key is stored securely in your browser and is never sent to our servers.</p>
